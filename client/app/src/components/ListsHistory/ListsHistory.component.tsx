...
export type ListHistoryComponentProps = {
  ...
  onListSelect: (list: ListEntity) => void
  pageSize?: number
}

const ListsHistoryComponent = ({
  lists,
  onAddListClick,
  onListSelect,
  pageSize = 12,
}: ListHistoryComponentProps) => {
  ...
  const onListClickHandler = useCallback<(list: ListEntity) => MouseEventHandler>(
    list => () => {
      onListSelect(list)
    },
    [onListSelect],
